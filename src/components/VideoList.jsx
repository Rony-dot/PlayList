                ...
                PlayList.map((item, i) => (
                    <ul key={i} className='list-group list-group-flush'>
                        ...
                        {SelectedItem.id === item.id && item.videos.map((video, j) => (
                            <li key={j} onClick={() => handleOnClick(video)} className="handle-overflow playlist-item px-3 py-2 list-group-item list-group-item-action">{video.title} <i class="bi bi-play-circle"></i>
                            </li>
                        ))}
                    </ul >
                ))
